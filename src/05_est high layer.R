...
## Title:    EST High sensitivity
## Purpose:  Process FBIS occurrence data for development of High level of EST
...
points_to_poly <- function(spp){
  ...
  spp_data <- occ_data_sf %>%
    ...
    filter(sampling_date > ymd("2002-01-01"))

   poly_id <- st_intersection(spp_data, fepa_rivers) %>%
     pull(UNIT_ID) %>%
     unique(.)

   high_polys <- fepa_rivers %>%
     filter(UNIT_ID %in% poly_id)

   pp_cast <- sf::st_cast(high_polys, "POLYGON")
   ...
   write_sf(pp_cast, glue("data output/screening tool layers/high/{spp}.shp"),
            delete_dsn = TRUE)
...
shpfiles <- dir("data output/screening tool layers/high", full.names = TRUE, pattern =".shp")
shpfiles

map(shpfiles,
    read_sf) %>%
   do.call(rbind, .) %>%
   write_sf("data output/screening tool layers/FWF_high.shp", delete_layer = TRUE,
            overwrite = TRUE)
